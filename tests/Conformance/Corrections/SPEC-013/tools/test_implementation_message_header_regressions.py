...
def test_every_header_declaration_has_the_common_specification_reference():
    ...
    for protocol, name in (("HTTP", "headers"), ("NATS", "headers"), ("MQTT/5.0", "user_properties"), ("KAFKA", "headers")):
        fields = protocols[protocol]["siblingattributes"]["protocoloptions"]["attributes"][name]["item"]["attributes"]
        assert fields["specurl"]["type"] == "uri"
        assert not fields["specurl"].get("required", False)
